...
/**
 * @typedef {import('svelte-motion/types/utils/transform')}
 */
import { transform } from 'svelte-motion/src/utils/transform';
/**
 * @typedef {import('svelte-motion/types/value/use-combine-values')}
 */
import { useCombineMotionValues } from 'svelte-motion/src/value/use-combine-values';
import type { DevMessage, ItemData, TransformOptions } from './types';
...
export const mixNumber = (from: number, to: number, progress: number) => from + (to - from) * progress;

export const isMotionValue = (value: any): value is MotionValue => Boolean(value && value.getVelocity());
