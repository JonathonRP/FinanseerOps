...
type CreateApiServerOptions<TRouter extends AnyRouter> = HTTPBaseHandlerOptions<TRouter, Request> &
	...
	...(!dev && {
		onError: ({ type, path, req, error }) => {
			...
			Sentry.captureException(
				new Error(
					`Encountered error while trying to process ${JSON.stringify(req, null, 2)}, ${type} @ ${path ?? '[no path]'}`
				)
			);
			throw error;
		},
