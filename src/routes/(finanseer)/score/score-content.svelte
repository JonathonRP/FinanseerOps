...
<script lang="ts">
	...
	const { comparison, comparisonScore$ } = getContext<{
		swap?: boolean;
		comparison?: Partial<{ score: number; positiveColor: Color; negativeColor: Color; swap: boolean }>;
		comparisonScore$?: Readable<number>;
	}>('ScoreCard');
...
<dd
	class={cn('flex w-[164.57px] items-start pt-1 text-gray-900 dark:text-gray-100', restProps.class, {
		'h-[64px]': children,
	})}>
	{#if children}
